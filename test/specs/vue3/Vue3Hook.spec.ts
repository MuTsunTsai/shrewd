...
describe('Vue3Hook', () => {
	...
	function reset() {
		...
		let V = Vue.defineComponent({
			template: `<div>{{d}}</div>`,
			setup() {
				const _switch = Vue.ref(true);
				const _a = Vue.computed(() => a);
				const _d = Vue.computed(() => {
					a.count++;
					return _switch.value ? _a.value.c + 1 : 0;
				});
				return { 'switch': _switch, 'a': _a, 'd': _d };
			}
		});
		let w = mount(V);
		return { a, ac, w };
	...
	describe('when autoCommit is on', () => {
		...
		it('renders without commit', async () => {
			...
			expect(a.count).to.equal(1);
			a.n = 5;
			await new Promise(resolve => setTimeout(resolve, 0)); // In Vue3, nextTick() runs faster than autoCommit
			await Vue.nextTick();
			expect(w.text()).to.equal("7");
			expect(a.count).to.equal(2);
		});
